...
const Clans = () => {
    ...
    return (
      <div className="page-enter max-w-4xl mx-auto p-4 md:p-6">
        ...
        <div className="rounded-lg overflow-hidden mb-6" style={{ border: `1px solid ${cColor}40` }}>
          <div className="h-32 relative" style={{ background: `linear-gradient(135deg, ${cColor}40, #080812)` }}>
            <div className="absolute bottom-0 left-0 right-0 p-6" style={{ background: 'linear-gradient(to top, #080812, transparent)' }}>
              <div className="flex items-end justify-between">
                <div>
                  <h1 className="font-['Orbitron'] text-3xl font-bold tracking-wider mb-1" style={{ color: cColor }}>
                    {detail.name} <span className="text-lg" style={{ color: '#6666AA' }}>[{detail.tag}]</span>
                  </h1>
                  <p className="font-['Rajdhani'] text-sm italic" style={{ color: '#6666AA' }}>"{detail.motto}"</p>
                </div>
                {isInClan && userRole === 'leader' && (
                  <Crown size={32} strokeWidth={1.5} style={{ color: cColor, opacity: 0.5 }} />
                )}
              </div>
            </div>
          </div>
          
          <div className="p-6" style={{ background: '#080812' }}>
            <div className="flex gap-4 flex-wrap mb-6">
              <span className="px-3 py-1 rounded text-xs font-['Rajdhani'] font-bold uppercase tracking-wider" style={{ background: `${cColor}20`, color: cColor, border: `1px solid ${cColor}40` }}>
                {detail.alignment}
              </span>
              <span className="flex items-center gap-1.5 font-['Rajdhani'] text-sm" style={{ color: '#6666AA' }}>
                <Users size={14} strokeWidth={1.5} /> {detail.members} members
              </span>
              <span className="flex items-center gap-1.5 font-['Rajdhani'] text-sm" style={{ color: '#00FF88' }}>
                <Trophy size={14} strokeWidth={1.5} /> {detail.wins}W / {detail.losses}L
              </span>
            ...
            <div>
              ...
              <div className="flex gap-2 mb-4">
                <button
                  ...
                  className={`px-4 py-2 rounded font-['Rajdhani'] font-semibold text-sm uppercase tracking-wider transition-all`}
                  style={{
                ...
                >
                  <TrendingUp size={14} strokeWidth={1.5} className="inline mr-1" /> Hot
                </button>
                <button
                ...
                >
                  <Clock size={14} strokeWidth={1.5} className="inline mr-1" /> New
                </button>
                <button
                ...
                >
                  <Trophy size={14} strokeWidth={1.5} className="inline mr-1" /> Top
                </button>
              </div>
              ...
              <div className="space-y-3">
                ...
                {sortedPosts.map(post => {
                  ...
                  return (
                    <div key={post.id} className="flex gap-3 p-4 rounded-lg" style={{ background: '#080812', border: '1px solid #1A1A2E' }}>
                      {/* Vote Section */}
                      <div className="flex flex-col items-center gap-1">
                        ...
                        >
                          <ArrowUp size={18} strokeWidth={1.5} style={{ color: '#00FF88' }} />
                        </button>
                        <span className="font-['Orbitron'] font-bold text-sm" style={{ color: score > 0 ? '#00FF88' : score < 0 ? '#FF003C' : '#6666AA' }}>
                          {score}
                        </span>
                        <button
                          ...
                          className="p-1 rounded hover:bg-[var(--bg-elevated)] transition-colors"
                        >
                          <ArrowDown size={18} strokeWidth={1.5} style={{ color: '#FF003C' }} />
                        </button>
                      </div>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          {post.isPinned && (
                            <Pin size={14} strokeWidth={1.5} style={{ color: '#FFD700' }} />
                          )}
                          <span className="px-2 py-0.5 rounded text-[10px] font-['Rajdhani'] font-bold uppercase" style={{ background: `${cColor}20`, color: cColor }}>
                            {post.type}
                          </span>
                        ...
                        <h3 className="font-['Orbitron'] font-bold text-base mb-2" style={{ color: '#E8E8FF' }}>
                          {post.title}
                        </h3>
                        <p className="font-['Rajdhani'] text-sm mb-3" style={{ color: '#6666AA' }}>
                          {post.body}
                        </p>
                        ...
                        <div className="flex gap-3">
                          <button className="flex items-center gap-1 text-xs font-['Rajdhani']" style={{ color: '#6666AA' }}>
                            <MessageCircle size={14} strokeWidth={1.5} /> {post.commentCount || 0} comments
                          </button>
                          {isLeaderOrOfficer && (
                            <>
                              <button
                                ...
                                style={{ color: '#6666AA' }}
                              >
                                <Pin size={14} strokeWidth={1.5} /> {post.isPinned ? 'Unpin' : 'Pin'}
                              </button>
                              <button
  ...
  return (
    <div className="page-enter max-w-5xl mx-auto p-4 md:p-6">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-['Orbitron'] text-2xl font-bold tracking-wider flex items-center gap-3" style={{ color: '#E8E8FF' }}>
          <Shield size={22} strokeWidth={1.5} /> CLANS
        </h1>
        <Button
